...
from pipeline.embedder import check_sentences_for_persona_job
from pipeline.generate_output import get_top_5_sections, get_top_5_sentence_groups_per_section
...
def process_trip_planning_input(input_data):
    ...
    from pipeline.generate_output import get_extracted_sections
...
def process_trip_planning_input(input_data):
    ...
    # Get extracted_sections (ExtractedSection)
    from pipeline.generate_output import get_top_5_sections as get_extracted_sections
    extracted_sections = get_extracted_sections(top5_avg_sections)
    extracted_sections_dicts = [sec.dict() for sec in extracted_sections]

    # Get subsection_analysis (SubsectionAnalysis)
    subsection_analysis = get_top_5_sentence_groups_per_section(top5_avg_sections)
    subsection_analysis_dicts = [sub.dict() for sub in subsection_analysis]
